...
void uniquePtrTest(void)
{
	...
	unique_ptr<int> p(new int);						//Tworzenie wskaünika - poprzez konstruktor
	unique_ptr<int> pp = make_unique<int>(5);		//Tworzenie wskaünika - poprzez make unique
	*p = 6;
	cout << "Value: " << *p << endl;
	*p = 10;
...
void sharedPtrTest(void)
{
	...
	shared_ptr<int> p(new int);						//Tworzenie wskaünika - poprzez konstruktor
	int w;
	shared_ptr<int> pp = make_shared<int>(w);		//Tworzenie wskaünika - poprzez make shared
	*p = 6;
	cout << "Value: " << *p << endl;
	*p = 10;
	cout << "Value changed: " << *p << endl;
